...
void delay_us(uint32_t nus)
{
    ...
    ticks = nus * HAL_RCC_GetHCLKFreq(); // 需要的节拍数
...
// size:字体大小
//*format:字符串起始地址
void LCD_ShowString(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint8_t size, const char *format, ...)
{
    uint8_t x0 = x, buf_index = 0;
    char buf[32];
    va_list aptr;
    

    va_start(aptr, format);
    vsprintf((char *)buf, format, aptr);
    va_end(aptr);
    ...
    while (buf[buf_index] <= '~' && buf[buf_index] >= ' ') // 判断是不是非法字符!
    {
        if (x >= width)
        {
            x = x0;
        ...
        LCD_ShowChar(x, y, buf[buf_index], size, 0);
        x += size / 2;
        buf_index++;
    }
}
